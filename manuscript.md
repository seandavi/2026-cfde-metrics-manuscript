...
These include the Cancer Research Data Commons [@doi:10.3389/fcell.2017.00083] (CRDC, NCI), the Cancer Genome Atlas (TCGA) [@pmid:24071849], the Trans-Omics for Precision Medicine program [@doi:10.1038/s41586-021-03205-y] (TOPMed, NHLBI), the All-of-Us Research Program [@doi:10.1056/NEJMsr1809937].
NIH has complemented data investments with cloud computing environments — the Cancer Genomics Cloud [@doi:10.1158/0008-5472.CAN-17-0387], BioData Catalyst [@doi:10.5281/zenodo.3822858], and AnVIL [@doi:10.1016/j.xgen.2021.100085] — designed to make analysis at scale accessible to any researcher.
The collective ambition is to democratize access to reusable data and accelerate scientific discovery.

Building upon the data coordinating centers spanning 19 Common Fund projects including GTEx [@doi:10.1038/ng.2653], the Human Microbiome Project [@doi:10.1038/nature11234; @doi:10.1038/nature11209], HuBMAP [@pmid:31597973], GlyGen [@doi:10.1093/glycob/cwz080], and Bridge2AI [@doi:10.1101/2024.03.11.584478], the Common Fund Data Ecosystem [@doi:10.1093/gigascience/giac105] (CFDE) is a cross-cutting initiative that harmonizes metadata across these diverse datasets and provides tooling for discovery and access.
...
The CFDE takes a slightly different approach from initiatives that aim to provide a single, unified access point for data.
The goal of the CFDE is to create an ecosystem of capabilities and tooling that enable high-value reuse of Common Fund datasets.
...
| :---- | :---- |
| Data Resource Center (DRC) | Enable researchers to discover CFDE data and other resources |
| Knowledge Center (KC) | Support analyses that provide cross-cutting biological insights through applications and outreach |
| Training Center (TC) | Address gaps in data and ecosystem understanding and drive a cultural shift required for cloud-based computing |
| Cloud Workspace (CWIC) | Provide computational power to analyze massive datasets in situ on large-scale shared infrastructure |
| Integration & Coordination (ICC) | Align CFDE centers into a unified ecosystem, drive data-driven decisions, and enhance CFDE sustainability |
...
Furthermore, we describe the ICC's implementation of an automated dashboard that reduces the burden on individual projects while enabling real-time monitoring of portfolio performance.

### Three Frameworks, One Lifecycle

No single metric — and no single framework — can capture the full picture of a data resource's value or trajectory throughout its life cycle.
We organize our approach around three complementary prioritization frameworks: *public value*, *scientific quality*, and *operations and finance*.
...
In the **early term** — infrastructure-building and data collection — *scientific quality* and *operations and finance* dominate.
...
Embedding this lifecycle logic into evaluation design, rather than applying uniform metrics across all phases, is a key practical contribution of this paper.

![Schematic relative priority of the three metric and evaluation frameworks--public value, scientific quality, and operations and finance--across the project lifecycle. Note that lower priority does **not** imply unimportant. ](images/figure1_lifecycle_curves.png){#fig:lifecycle_curves}

The *public value* framework captures the outputs and impacts of a data resource, software tool, or outreach program, or other ecosystem component: user engagement, data downloads, funded grants, publications, and downstream scientific influence.
The *scientific quality* framework measures reusability and long-term value, guided by the FAIR principles (Findable, Accessible, Interoperable, Reusable) [@doi:10.1038/sdata.2016.18; @doi:10.1038/sdata.2018.118] and encompassing the metadata, documentation, and standardization needed for data to be reliably reused.
The *operations and finance* framework covers the infrastructure and cost considerations — from server uptime and compute performance to the sustainability of the resource beyond the initial funding period and potentially the robustness to the sunsetting of funding.
...
Website engagement metrics — *page views*, *time on page*, *actions/triggers*, *priority link clicks*, and *new vs. returning users* — are easy to collect and track trends over time.
While they can provide early-stage insight into potential for uptake, they provide limited insight into actual data use.
Combining *time on page* with *actions/triggers* offers a more informative measure of engagement.
Key limitations include that bots can skew counts and VPN use in academic settings confounds new-versus-returning user tracking.
In an era of more sophsticated AI systems, bot-driven traffic may transform from strictly noise to be corrected to an additional indicator of resource value.
...
Publications and citations, accessible via sites like PubMed [@https://pubmed.ncbi.nlm.nih.gov], Scopus [@https://www.scopus.com], Google Scholar [@https://scholar.google.com], Clarivate [@https://clarivate.com], and OpenAlex [@https://openalex.org], provide strong proxies of scientific impact.
...
In the context of journal scoring, more sophsticated alternatives add weighting to account for various properties of the citing article — both better capture the prestige and subject-specific impact of data repositories than raw citation counts [@doi:10.4103/1008-682X.171582].
...
In the context of scientific software, which is also a key CFDE deliverable, *public value* includes many of the same metrics as data, including *downloads* and citation-related metrics that relate to manuscripts describing the software.
Assessments of the *public value* of scientific software can also incorporate markers of engagement on software development platforms, which include elements such as *stars*, *forks*, and *issues*.
...
The most profound long-term measure of public value is *practice change* — a fundamental shift in how science is conducted, moving from hypothesis-driven research to data-driven discovery and from evidence-based practice to practice-based evidence [@doi:10.15265/IY-2014-0020].
Concrete indicators include the adoption rate of cloud-based workspaces versus local data downloads (a proxy for computational paradigm shift), cross-disciplinary co-authorship networks across Common Fund programs (a measure of new collaborative partnerships), and the geographic and institutional breadth of the user base, particularly the participation of early-stage investigators from under-resourced institutions.
